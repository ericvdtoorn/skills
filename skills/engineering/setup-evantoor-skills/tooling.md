...
- **Running commands**: prefer `mise run <task>` if a matching task exists. For ad-hoc commands, use `mise exec -- <cmd>` so you pick up the pinned tool versions.
...
2. If one matches the user's intent, prefer `mise run <task>` over a raw command.
3. If no task matches, run the command with `mise exec -- <cmd>`. Consider creating a task (`/mise-tasks` skill) if the user is likely to want it again.
