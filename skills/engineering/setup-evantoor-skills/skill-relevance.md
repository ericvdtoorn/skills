...
- evantoor core workflow: `grill-me`, `grill-with-docs`, `to-prd`, `to-issues`, `triage`, `tdd`, `diagnose`, `improve-codebase-architecture`, `zoom-out`, `prototype`, `handoff`, `write-a-skill`, `caveman`, `gitmoji-commits`, `comment-style`
- doc fetchers / general tooling: `context7` (MCP), and similar library-docs lookups

`tdd`, `diagnose`, `to-issues`, `triage` are language-agnostic by design — a repo being Rust or Python is **not** a reason to disable them.
