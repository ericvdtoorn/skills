...
- **[batch-grill-me](./batch-grill-me/SKILL.md)** — A relentless interview that asks every frontier question at once, round by round, recomputing the frontier as each round's answers settle decisions.
- **[claude-handoff](./claude-handoff/SKILL.md)** — Hand the current conversation off to a fresh background `claude --bg` agent seeded with the handoff summary, instead of saving a document.
- **[to-questionnaire](./to-questionnaire/SKILL.md)** — Turn a decision you can't answer alone into a Markdown questionnaire for someone else to fill in — grills you about the send, not the subject.
